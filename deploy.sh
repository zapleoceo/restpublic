...
log "🚀 ========================================="
log "🚀 НАЧИНАЕМ ИСПРАВЛЕННЫЙ ДЕПЛОЙ v5.5"
log "🚀 ========================================="
log "📅 Время начала: $(date)"
log "💻 Система: $(uname -a)"
...
# Проверяем изменения в файлах данных и статических ресурсах
...
# Пауза для избежания rate limiting Telegram API
log "⏳ Пауза для избежания rate limiting Telegram API..."
sleep 10
...
# Проверяем бота с обработкой rate limiting
if pm2 list | grep -q "northrepublic-bot.*online"; then
    log "✅ Bot процесс запущен"
else
    log "⚠️ Bot процесс не запущен - проверяем причину..."
    
    # Проверяем логи бота на наличие ошибок конфликта или rate limiting
    ...
    if echo "$bot_logs" | grep -q "409.*Conflict\|terminated by other getUpdates request"; then
        log "🔄 Обнаружена ошибка конфликта - игнорируем ошибку бота"
        log "📋 Telegram API конфликт: другой экземпляр бота еще активен"
        log "⏰ Рекомендуется подождать 1-2 минуты перед следующим деплоем"
        log "⚠️ Bot не запущен, но деплой продолжается"
    elif echo "$bot_logs" | grep -q "429.*Too Many Requests\|retry after"; then
        log "🔄 Обнаружена ошибка rate limiting - игнорируем ошибку бота"
        log "📋 Telegram API требует паузу. Ошибка записана в логи"
        log "⏰ Рекомендуется подождать 8-10 минут перед следующим деплоем"
        log "⚠️ Bot не запущен, но деплой продолжается"
    else
        log "❌ Bot не запущен по другой причине"
        pm2 list
        log "📋 Логи bot:"
        pm2 logs northrepublic-bot --lines 5 || log "❌ PM2 логи недоступны"
        exit 1
    fi
fi
...
log "🚀 ========================================="
log "🚀 ИСПРАВЛЕННЫЙ ДЕПЛОЙ v5.5 ЗАВЕРШЕН!"
log "🚀 ========================================="
