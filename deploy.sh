...
# Проверяем наличие основных файлов меню (опционально)
if [ ! -f "menu.php" ]; then
    warning "menu.php не найден в корне (опциональный файл)"
fi

if [ ! -f "menu2.php" ]; then
    warning "menu2.php не найден в корне (опциональный файл)"
fi
...
# Создаем папку для изображений событий если её нет
if [ ! -d "images/events" ]; then
    log "📁 Создаю папку images/events..."
    mkdir -p images/events
    chmod 755 images/events
    echo "Options -Indexes" > images/events/.htaccess
    success "Папка images/events создана"
fi

# Создаем папку для логов если её нет
if [ ! -d "logs" ]; then
    log "📁 Создаю папку logs..."
    mkdir -p logs
    chmod 755 logs
    success "Папка logs создана"
fi
