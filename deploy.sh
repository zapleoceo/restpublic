...
if pm2 list | grep -q "northrepublic-bot.*online"; then
    ...
    # Проверяем логи бота на наличие rate limiting ошибки
    bot_logs=$(pm2 logs northrepublic-bot --lines 10 2>/dev/null || echo "")
    if echo "$bot_logs" | grep -q "429.*Too Many Requests\|retry after"; then
        log "🔄 Обнаружена ошибка rate limiting - пропускаем запуск бота"
        log "📋 Telegram API требует паузу. Bot будет запущен автоматически позже"
        log "⏰ Рекомендуется подождать 8-10 минут перед следующим деплоем"
        
        # Планируем перезапуск бота через 10 минут
        log "⏰ Планируем перезапуск бота через 10 минут..."
        (sleep 600 && pm2 restart northrepublic-bot) &
        log "✅ Перезапуск бота запланирован"
    else
