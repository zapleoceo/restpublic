...
class Logger {
    ...
    /**
     * Получение количества логов за определенный период
     */
    public function getLogsCount($options = []) {
        try {
            $hours = $options['hours'] ?? 24;
            $cutoffTime = time() - ($hours * 3600);
            
            if ($this->useMongoDB) {
                $cutoffDate = new MongoDB\BSON\UTCDateTime($cutoffTime * 1000);
                return $this->logsCollection->countDocuments([
                ...
                foreach ($logs as $log) {
                    if (isset($log['timestamp'])) {
                        $logTime = is_string($log['timestamp']) ? strtotime($log['timestamp']) : $log['timestamp'];
            ...
            error_log("Error getting logs count: " . $e->getMessage());
            return 0;
        }
    }
}
