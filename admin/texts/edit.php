...
try {
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        // Проверяем, не существует ли уже такой ключ (кроме текущего)
        $existing = $textsCollection->findOne([
            'key' => $key,
            '_id' => ['$ne' => new MongoDB\BSON\ObjectId($textId)]
        ]);
        ...
        $result = $textsCollection->updateOne(
            ['_id' => new MongoDB\BSON\ObjectId($textId)],
            ['$set' => $updateData]
        );
        
        if ($result->getModifiedCount() > 0) {
            // Логируем обновление
            logAdminAction('update_text', 'Обновлен текст', [
                'key' => $key,
                'category' => $category,
                'text_id' => $textId,
                'changes' => array_diff_assoc($updateData, $text->toArray())
            ]);
            
            $success = 'Текст успешно обновлен!';
            
            // Обновляем данные для отображения
            $text = $textsCollection->findOne(['_id' => new MongoDB\BSON\ObjectId($textId)]);
        } else {
            throw new Exception('Ошибка при обновлении текста');
        }
    }
...
<head>
    ...
    <title>Редактировать текст - North Republic Admin</title>
    ...
    <div class="admin-container">
        ...
        <main class="admin-main">
            ...
            <div class="form-container">
                <div class="text-info">
                    <h4>Информация о тексте</h4>
                    <p>
                        Создан: <?php echo $text['created_at']->toDateTime()->format('d.m.Y H:i'); ?> | 
                        Обновлен: <?php echo $text['updated_at']->toDateTime()->format('d.m.Y H:i'); ?> | 
                        Автор: <?php echo htmlspecialchars($text['created_by'] ?? 'Unknown'); ?>
                    </p>
                </div>
                ...
                <form method="POST" class="card">
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" id="key" name="key" 
                               value="<?php echo htmlspecialchars($text['key'] ?? ''); ?>" 
                    ...
                    <div class="form-group">
                        ...
                        <select id="category" name="category" required>
                            ...
                            <option value="intro" <?php echo ($text['category'] ?? '') === 'intro' ? 'selected' : ''; ?>>Главная страница</option>
                            <option value="about" <?php echo ($text['category'] ?? '') === 'about' ? 'selected' : ''; ?>>О нас</option>
                            <option value="menu" <?php echo ($text['category'] ?? '') === 'menu' ? 'selected' : ''; ?>>Меню</option>
                            <option value="gallery" <?php echo ($text['category'] ?? '') === 'gallery' ? 'selected' : ''; ?>>Галерея</option>
                            <option value="footer" <?php echo ($text['category'] ?? '') === 'footer' ? 'selected' : ''; ?>>Подвал</option>
                            <option value="header" <?php echo ($text['category'] ?? '') === 'header' ? 'selected' : ''; ?>>Шапка</option>
                            <option value="buttons" <?php echo ($text['category'] ?? '') === 'buttons' ? 'selected' : ''; ?>>Кнопки</option>
                            <option value="errors" <?php echo ($text['category'] ?? '') === 'errors' ? 'selected' : ''; ?>>Ошибки</option>
                            <?php foreach ($categories as $category): ?>
                                <?php if (!in_array($category, ['intro', 'about', 'menu', 'gallery', 'footer', 'header', 'buttons', 'errors'])): ?>
                                    <option value="<?php echo htmlspecialchars($category); ?>" 
                                            <?php echo ($text['category'] ?? '') === $category ? 'selected' : ''; ?>>
                        ...
                        <div class="translation-group">
                            <div class="form-group">
                                ...
                                <textarea id="translation_ru" name="translation_ru" rows="4" 
                                          placeholder="Введите текст на русском языке" required><?php echo htmlspecialchars($text['translations']['ru'] ?? ''); ?></textarea>
                            </div>
                        </div>
                    </div>
                        ...
                        <div class="translation-group">
                            <div class="form-group">
                                ...
                                <textarea id="translation_en" name="translation_en" rows="4" 
                                          placeholder="Enter text in English"><?php echo htmlspecialchars($text['translations']['en'] ?? ''); ?></textarea>
                            </div>
                        </div>
                    </div>
                        ...
                        <div class="translation-group">
                            <div class="form-group">
                                ...
                                <textarea id="translation_vi" name="translation_vi" rows="4" 
                                          placeholder="Nhập văn bản bằng tiếng Việt"><?php echo htmlspecialchars($text['translations']['vi'] ?? ''); ?></textarea>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="form-actions">
                        ...
                        <div>
                            <a href="delete.php?id=<?php echo $textId; ?>" class="btn-delete" 
                               onclick="return confirm('Вы уверены, что хотите удалить этот текст?')">Удалить</a>
                        </div>
                    </div>
                </form>
            </div>
